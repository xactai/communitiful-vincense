        ...
        <div className="flex space-x-2 border-b border-gray-200 dark:border-gray-700 mb-6 overflow-x-auto pb-1">
            {keys.map(key => (
                <button
                    ...
                    className={`py-2 px-4 rounded-t-lg font-medium text-sm whitespace-nowrap transition-colors ${
                        activeSheet === key
            ...
            <div className="overflow-x-auto mb-6">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                    <thead className="bg-gray-50 dark:bg-gray-800">
                        <tr>
                            {headers.map(h => (
                                <th key={h} className="px-3 py-3 text-left font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                    {h}
                                </th>
                            ))}
                        </tr>
                    ...
                    <tbody className="bg-white dark:bg-card-bg-dark divide-y divide-gray-200 dark:divide-gray-700">
                        {displayData.slice(0, 50).map((row, i) => ( // Show first 50 rows for perf
                ...
                <div className="mt-2 text-xs text-gray-500 text-center">Showing first 50 rows</div>
            </div>
