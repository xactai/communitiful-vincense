...
export const CircumstanceAnalyticsTab: React.FC<Props> = ({ data, isDarkMode }) => {
    ...
    return (
        <div className="space-y-8 p-4">
            ...
            {vitals.map(vital => (
                <div key={vital.key} className="bg-white dark:bg-card-bg-dark p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    <h3 className="text-xl font-bold mb-6 text-indigo-900 dark:text-indigo-400 flex items-center gap-2">
                        {vital.label} <span className="text-sm font-normal text-gray-500">({vital.unit})</span>
                    </h3>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                ...
                                data={[
                                    ...
                                    ...['VinCense', 'Dr Trust', 'Dr Odin'].map(dev => {
                                        ...
                                        sortedCircumstances.forEach(c => {
                                            const vals = processed[vital.key]?.[c]?.[dev] || [];
                                            if (vals.length > 0) {
                        ...
                        <div className="col-span-1 lg:col-span-2 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border-l-4 border-gray-400 mb-2">
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                                This chart compares the average {vital.label} across different activities (Resting, Walking, etc.) and devices.
                            </p>
                        </div>
                        {/* Insight */}
                        ...
                        <div className="h-[500px] overflow-x-auto w-full border border-gray-100 dark:border-gray-800 rounded-lg">
                            ...
                            <div style={{ width: '1400px', height: '100%' }}>
                                <Plot
                                    data={[
                                        // 1. Box Plots First
                                        ...sortedCircumstances.map(c => {
                                            const vals = processed[vital.key]?.[c]?.['VinCense'] || [];
                                            return {
                                                y: vals,
                                                name: c,
                                                type: 'box',
