...
export const TrustOdinComparisonTab: React.FC<Props> = ({ data, isDarkMode }) => {
    ...
    return (
        <div className="space-y-12 p-4">
            <h2 className="text-2xl font-bold text-blue-900 dark:text-blue-400 mb-6">Dr Trust vs Dr Odin Analysis</h2>

            {['Pulse', 'SpO2', 'Resp'].map(vital => {
                const stats = analytics[vital];
                if (!stats) return null;

                const { data: pts, mae, diffDistribution, within2Percent, total, meanDiff, sdDiff, byCircumstance } = stats;

                const unit = vital === 'Pulse' ? 'bpm' : vital === 'SpO2' ? '%' : 'rpm';

                return (
                    <div key={vital} className="space-y-8 border-b border-gray-200 dark:border-gray-700 pb-12 last:border-0">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-xl font-bold text-indigo-800 dark:text-indigo-300">{vital === 'Resp' ? 'Respiratory Rate' : vital} Analysis</h3>
                            {/* KPI Card */}
